...
static void data_handler(AccelData *data, uint32_t num_samples) {
  if (i%3!=0){
    ...
    snprintf(data_buffer, BFSIZE, "{'x':%d,'y':%d,'z':%d,'t':%s%03d}", data[0].x, data[0].y, data[0].z, timebuf,end);
 
APP_LOG(APP_LOG_LEVEL_DEBUG, "{'x':%d,'y':%d,'z':%d,'t':%s%03d}", data[0].x, data[0].y, data[0].z, timebuf,end);
